...
export default function ScatteredSection() {
  ...
  return (
    <section id="secrets" className="relative bg-[#fcfcfc] scroll-mt-24">
      ...
      <div className="border-x border-[#efefef] mx-auto max-w-[1400px] px-8 sm:px-10 lg:px-14 py-16 lg:py-24 relative">
        ...
        <StaggerContainer className="flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-20">
          <motion.div variants={scaleFadeIn()} className="flex-1 max-w-lg">
            <div className="text-[#FF3B30] text-xs uppercase font-bold tracking-widest mb-3">The problem</div>
          ...
          <motion.div variants={scaleFadeIn()} className="flex flex-wrap justify-center gap-5">
            {sources.map((source, i) => (
              <motion.div
                key={i}
                className="bg-[#F5F5F7] p-5 rounded-[20px] w-[220px] shadow-[0_2px_20px_rgba(0,0,0,0.06)] border border-[#E5E5EA] relative cursor-default"
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-8 h-8 rounded-lg bg-white shadow-sm flex items-center justify-center">
                    <source.icon className="w-4 h-4 text-[#1D1D1F]" />
                  </div>
                  <span className="text-[#1D1D1F] font-medium text-sm">{source.label}</span>
                </div>
                <div className="text-[10px] leading-relaxed bg-white/60 rounded-md px-2 py-1.5 font-mono text-[#6E6E73] inline-block">
                  {source.desc}
                </div>
                <div className="absolute -top-2 -right-2 px-2 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider bg-[#FF3B30]/10 text-[#FF3B30]">
                  {source.status}
                </div>
              </motion.div>
            ))}
