...
const features = [
  { title: "Centralized Secret Storage", description: "Store all API keys, tokens, and credentials in one secure vault." },
  { title: "Version History", description: "Track every change with full audit logs and rollback support." },
  { title: "Cross-Environment Sync", description: "Sync secrets across development, staging, and production automatically." },
  { title: "Real-Time Validation", description: "Detect missing, expired, or misconfigured secrets instantly." },
];
    ...
    <SectionShell id="problem">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="rounded-2xl border border-[#ededed] bg-white p-8 shadow-sm flex flex-col">
          ...
          <StaggerContainer staggerDelay={0.1} margin="-60px" className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-auto">
            {features.map((feat, idx) => (
              <motion.div
                key={idx}
                variants={fadeInUp(20, 0.4)}
                className="rounded-xl border border-[#efefef] p-4 hover:border-[#191919]/25 transition-all"
              >
