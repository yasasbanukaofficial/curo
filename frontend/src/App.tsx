  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-6 text-slate-900">
      <div className="w-full max-w-xl rounded-2xl bg-white p-10 text-center shadow-sm ring-1 ring-slate-200">
        ...
        <form onSubmit={login} className="mt-6 space-y-4 text-left">
          <input
            type="email"
            value={email}
          ...
          <button
            type="submit"
            className="w-full rounded-xl bg-indigo-600 px-6 py-3 text-sm font-medium text-white shadow-sm transition hover:bg-indigo-500 active:scale-[0.98]"
          >
            Sign in with Email
          </button>
        </form>
        ...
        <button
          ...
          className="mt-4 flex w-full cursor-pointer items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 active:scale-[0.98]"
        >
...
export default function App() {
  ...
  const [loggedIn, setLoggedIn] = useState(
    path === "/dashboard" || localStorage.getItem("loggedIn") === "true",
  );
